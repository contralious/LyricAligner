...
function PopupLyrics() {
	...
	const LyricProviders = {
        ...
        async fetchLocal(info) {
			try {
				...
				const response = await fetch(`http://localhost:8080/lyrics?${params.toString()}`);
				if (!response.ok) {
					return { error: "Local lyrics not found or server offline" };
				}
				const data = await response.json();
				if (data?.lyrics && data.lyrics.length > 0) {
					return { lyrics: data.lyrics };
				}
				return { error: "No local lyrics" };
			} catch (e) {
				return { error: "Local lyrics server offline" };
			}
		},
	};
	...
	const userConfigs = {
		...
		services: {
            local: {
				on: boolLocalStorage("popup-lyrics:services:local:on", true),
				call: LyricProviders.fetchLocal,
				desc: "Locally generated & AI-aligned fine-grained lyrics via localhost server.",
			},
			netease: {
		...
		if (CACHE?.[info.uri]?.lyrics?.length && !refresh) {
			...
			for (const name of userConfigs.servicesOrder) {
				...
				try {
					...
					if (!sharedData.error) {
						// 1. Abort any previous alignment download immediately on skip!
						if (window.alignAbortController) {
							window.alignAbortController.abort();
							window.alignAbortController = null;
						}

						const isChunky = sharedData.lyrics?.some(l => l.text.length > 50 || l.text.includes(","));
						if (isChunky) {
							// Create a fresh abort controller for this song
							const controller = new AbortController();
							window.alignAbortController = controller;

							fetch("http://localhost:8080/align", {
								method: "POST",
								...
								body: JSON.stringify({
									title: info.title,
									artist: info.artist,
									lines: sharedData.lyrics.map(l => l.text)
								})
							})
							.then(r => r.json())
							.then(res => {
								// Only swap if the song is STILL currently playing!
								if (res?.lyrics?.length && Player.data?.item?.metadata?.title === info.title) {
									console.log("[PopupLyrics] Seamlessly swapping to AI aligned lyrics!");
									sharedData.lyrics = res.lyrics;
									CACHE[info.uri] = sharedData;
									renderState = null;
									tick(userConfigs);
								}
							...
							.catch(err => {
								if (err.name !== "AbortError") {
									console.log("[PopupLyrics] Alignment skipped or server offline");
								}
							});
						}
						return;
